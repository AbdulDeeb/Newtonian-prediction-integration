...
float cueCenter1_x = 0;
float cueCenter1_y = 0;
float cueCenter1_z = 0;

float cueCenter2_x = 0;
float cueCenter2_y = 0;
float cueCenter2_z = 0;


float cueRadius = 16;


int goalDepth = 100; //idk what you are, but w/e

int ResponseType;
double BallDirection; 
float responseAngle = -1.5707; //90 degrees
float responseSquare_x = 0;
float responseSquare_z  = 0; //= displayDepth - 28
float responseSquare_y = 0; 
float responseMag = 28;
float responseDif_x;
float responseDif_z;
...
string experiment_directory = "R:/CLPS_Domini_Lab/abdul/fall19-abdul-billiardsSingleBallControl/";
string experiment_directory_desktop = "C:/Users/visionlab/Desktop/abdul/billiardsSingleBallControl/";
// parameters file directory and name
string parametersFile_directory = experiment_directory + "fall19-abdul-billiardsSingleBallControlParameters.txt";
// trial file headers
string trialFile_headers = "subjName\ttrialN\tocclusion\tspeed\tdeflectionAngle\telapsed\tframeN\ttargetCenter_x\ttargetCenter_y\ttargetCenter_z\tResponseType\tBallDirection\tresponseAngle";
/*************************** FUNCTIONS ***********************************/
...
void initStreams()
{
	...
	ResponseType = str2num<int>(parameters.find("ResponseType"));
	// trialFile directory
...
void handleKeypress(unsigned char key, int x, int y){
	switch (key){
		...
		case '2': //  left
		{
		if (responseAngle <= 0 && responseAngle >= -3.1415){
			if(ResponseType ==0) {
				responseAngle = (180*responseAngle)/3.1415+2 ;
				responseAngle = DEG2RAD*responseAngle;
				responseSquare_x = (cos(responseAngle)*responseMag) + 0;
				responseSquare_z = (sin(responseAngle)*responseMag)- 400;
			}
			else if(ResponseType == 1){
				responseAngle = (180*responseAngle)/3.1415+ 2;
				responseAngle = DEG2RAD*responseAngle;
				responseSquare_x = (cos(responseAngle)*responseMag) + 0;
				responseSquare_y = (sin(responseAngle)*responseMag) -86;
			}
		}
		else if (responseAngle > 0){
				responseAngle = 0;
				beepOk(3);	
			}
		else{
				responseAngle = -3.1415;
				beepOk(3);	
				}
		...
		case '+':
		{
			if (elapsed>(delay+occlusion)&& !(response == 2)){
				response++;
				if (response == 2){
					responseAngle = ((180*responseAngle)/3.1415)+90;
					advanceTrial();
					beepOk(19);
				}
				else { 
...
void drawInfo()
{
	...
	if ( visibleInfo )
	{
		...
		if (finished) {
			...
			text.draw("# Response Type:" +stringify<int>(ResponseType));
			text.draw("# Hand At Start? " +stringify<double>(handAtStart));
			text.draw("# Occlusion " +stringify<int>(occlusion));
			text.draw("# Ball Direction " +stringify<double>(BallDirection));
			text.draw("# responseSquare_x " +stringify<float>(responseSquare_x));
			text.draw("# responseSquare_z " +stringify<float>(responseSquare_z));
			text.draw("# responseSquare_y " +stringify<float>(responseSquare_y));
			text.draw("# response Angle Corrected" +stringify<float>(((180*responseAngle)/3.1415)+90)); //*-180/3.1415
			text.draw("# deflectionAngle" +stringify<float>(deflectionAngle));
...
void drawStimulus()
{
	if((occlusion == 40 || occlusion == 80) && elapsed<(occlusion + delay))  // 1. in partial occlusion, during display period
	{	
		...
		glPopMatrix();
		 ...
		 //2. Draw referenc ball 1
		//glDisable(GL_LIGHTING);
		glColor3f(0.8,0.0,0.0);
		glPushMatrix();
		glLoadIdentity();
		GLUquadricObj* cueBall_1 = gluNewQuadric();
		gluQuadricDrawStyle(cueBall_1, GLU_FILL);
		glMaterialfv(GL_FRONT, GL_SPECULAR, specularMaterial);
		glMaterialf(GL_FRONT, GL_SHININESS, shininessMaterial);
		glTranslated(cueCenter1_x,cueCenter1_y,cueCenter1_z);
		gluSphere(cueBall_1, cueRadius, 64, 64);
		gluDeleteQuadric(cueBall_1);
		glPopMatrix();

		//2. Draw referenc ball 2
		//glDisable(GL_LIGHTING);
		glColor3f(0.8,0.0,0.0);
		glPushMatrix();
		glLoadIdentity();
		GLUquadricObj* cueBall_2 = gluNewQuadric();
		gluQuadricDrawStyle(cueBall_2, GLU_FILL);
		glMaterialfv(GL_FRONT, GL_SPECULAR, specularMaterial);
		glMaterialf(GL_FRONT, GL_SHININESS, shininessMaterial);
		glTranslated(cueCenter2_x,cueCenter2_y,cueCenter2_z);
		gluSphere(cueBall_2, cueRadius, 64, 64);
		gluDeleteQuadric(cueBall_2);
		glPopMatrix();
	...
	// draw response plane
	if(elapsed>=(occlusion + responseDelay + delay)) //  after display period
		{
		if(ResponseType == 0){
			glLoadIdentity();
			glPushMatrix();
			...
			glBegin(GL_LINES);
			glVertex3f(0 ,-86,displayDepth);
			glVertex3f(responseSquare_x,-86,responseSquare_z);
			glEnd();
			glLineWidth(1.0);
			...
			glBegin(GL_LINES);
			glVertex3f(0 ,-86,displayDepth);
			glVertex3f(responseSquare_x,responseSquare_y,displayDepth);
			glEnd();
			glLineWidth(1.0);
...
void initTrial()
{
	...
	occlusion = trial.getCurrent()["Occlusion"];
	int aimingAngle_index = trial.getCurrent()["AimingAngle"];
	int deflectionAngle_index = trial.getCurrent()["DeflectionAngle"];

	

	cueCenter1_x = 0;
	cueCenter1_y = -122;
	cueCenter1_z = displayDepth;
	
	cueCenter2_x = 0;
	cueCenter2_y = -28;
	cueCenter2_z = displayDepth;
	...
	responseAngle = -1 + -1*rand()%179;
	responseAngle = DEG2RAD*responseAngle;
	responseSquare_x = (cos(responseAngle)*responseMag) + 0;
	responseSquare_z = (sin(responseAngle)*responseMag) + displayDepth;
	responseSquare_y = (sin(responseAngle)*responseMag) -86;
	//Response plane

	//float responseDif_x = targetCenter_x - responseSquare_x;
	//float responseDif_z = targetCenter_z- responseSquare_z; 
	//responseMag = sqrt(pow(responseDif_x,2) + pow(responseDif_z,2));

	switch(deflectionAngle_index){
		case 0: //-45.72
			deflectionAngle= -45.72;
			targetVel_x_temp = -1;
			targetVel_z_temp = 1/tan(deflectionAngle*DEG2RAD);
			BallDirection = 1;
			break;					
			
		case 1: //-22.86 (Newtonian)
