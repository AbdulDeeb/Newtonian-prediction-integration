...
string experiment_directory = "R:/CLPS_Domini_Lab/abdul/spring18-billiardsCntrl_staircase/";
// parameters file directory and name
string parametersFile_directory = experiment_directory + "spring18-billiardsCntrl_staircaseParameters.txt";
// trial file headers
...
void drawStimulus()
{
	if(((occlusion == 40 || occlusion == 80) && elapsed<(timeOfContact+occlusion)) || // 1. in partial occlusion, during display period
		(!contactOccurred)) 
	{	
	...
	// draw response plane
	if(elapsed>=(timeOfContact+occlusion+ responseDelay)) //  after display period
...
void initTrial()
{
	...
	response = 0;
	string subjectName = parameters.find("SubjectName");
	string trialFileName = experiment_directory + subjectName + "/" + subjectName + "_trial" + stringify<double>(trialNumber) + ".txt";
	...
	responseSquare_z = targetCenter_z - 28;
