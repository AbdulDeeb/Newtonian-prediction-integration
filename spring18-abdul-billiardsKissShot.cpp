...
string experiment_directory = "R:/CLPS_Domini_Lab/abdul/spring18-billiardsKissShot/";
string experiment_directory_desktop = "C:/Users/visionlab/Desktop/abdul/kissShot/";
// parameters file directory and name
string parametersFile_directory = experiment_directory + "spring18-billiardsKissShotParameters.txt";
// trial file headers
string trialFile_headers = "subjName\ttrialN\tactivePhase\tocclusion\taimingAngle\tspeed\tdeflectionAngle\tresponse\ttimeOfContact\telapsed\tframeN\tcueBallStruck\tcueVelSet\tcontactOccurred\tcueCenter_x\tcueCenter_y\tcueCenter_z\ttargetCenter_x\ttargetCenter_y\ttargetCenter_z\tindXraw\tindYraw\tindZraw\tresponseAngle";
/*************************** FUNCTIONS ***********************************/
...
void handleKeypress(unsigned char key, int x, int y){
	switch (key){
		...
		case '+':
		{
			if (elapsed>(timeOfContact+occlusion)&& contactOccurred && !(response == 2)){
				response++;
				if (response == 2){
					advanceTrial();
					beepOk(19);
...
void drawStimulus()
{
	if(((occlusion == 40 || occlusion == 80) && elapsed<(timeOfContact+occlusion + delay)) && // 1. in partial occlusion, during display period
		(contactOccurred)) 
	{	
	...
	// draw response plane
	if(elapsed>=(timeOfContact + occlusion + responseDelay + delay)) //  after display period
...
void initTrial()
{
	...
	switch(aimingAngle_index){ 
		case 0: //Left
			...
			goalAngle= 22.86;
			goalX_offset = tan(goalAngle*DEG2RAD)*goalDepth;
			indXNow = 35.27;
			indYNow = -75;
			indZNow = -200;
			movementX = -0.3527;
	...
	switch(deflectionAngle_index){
		...
		case 1: //-38.10
			deflectionAngle= -38.10;
			targetVel_x_temp = -1;
			targetVel_z_temp = 1/tan(deflectionAngle*DEG2RAD);
			break;

		case 2:  //-30.48
			deflectionAngle= -30.48;
			targetVel_x_temp = -1;
			targetVel_z_temp = 1/tan(deflectionAngle*DEG2RAD);
			break;
		...
		case 4: //-15.24
			deflectionAngle= -15.24;
			targetVel_x_temp = -1;
			targetVel_z_temp = 1/tan(deflectionAngle*DEG2RAD);
			break;

		case 5:  //-7.62
			deflectionAngle= -7.62;
			targetVel_x_temp = -1;
			targetVel_z_temp = 1/tan(deflectionAngle*DEG2RAD);
			break;
...
void online_trial()
{
	...
	if (fingersCalibrated && !finished)
	{
		...
		if(contactOccurred && elapsed >= timeOfContact +delay  ){
